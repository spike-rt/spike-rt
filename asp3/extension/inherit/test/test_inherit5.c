 ...
 *  の責任を負わない．
 * 
 *  $Id: test_inherit5.c 1618 2022-09-19 02:57:29Z ertl-hiro $
 */
 ...
 * 【テストの目的】
 *
 *  タスクの待ち解除（wait_dequeue_wobj）時の優先度継承ミューテックス
 *  による優先度制御についてテストする．
 *
 * 【テスト項目】
 *
 *	(A) 自タスクAがロックしている優先度継承ミューテックスXに対してロッ
 *		ク待ち状態となっているタスクBを強制的に待ち解除（rel_wai）した
 *		ことに伴う，タスクAの優先度制御
 *		(A-1) ミューテックスXに，自タスクAと同じ現在優先度のタスクBが
 *			  待ち状態となっており，タスクBの強制的な待ち解除で自タス
 ...
 *			  と
 *		(A-3) ミューテックスXに，自タスクAより低い現在優先度のタスクB
 *			  が待ち状態となっている場合に，タスクBの強制的な待ち解除
 *			  で自タスクの現在優先度が変わらないこと．また，同じ優先度
 *			  内での優先順位が変わらないこと
 *	(B) タスクA（実行可能状態）がロックしている優先度継承ミューテック
 *		スXに対してロック待ち状態となっているタスクBを強制的に待ち解除
 *		（rel_wai）したことに伴う，タスクAの優先度制御
 *		(B-1) ミューテックスXに，タスクAと同じ現在優先度のタスクBが待
 *			  ち状態となっており，タスクBの強制的な待ち解除でタスクAの
 *			  現在優先度が下がるべき場合に，現在優先度が適切に下げられ
 *			  ること．また，同じ優先度内での優先順位が最高になること
 *		(B-2) ミューテックスXに，タスクAと同じ現在優先度のタスクBが待
 *			  ち状態となっており，タスクBの強制的な待ち解除でタスクAの
 *			  現在優先度が変わるべきでない場合に，現在優先度が変わらな
 *			  いこと．また，同じ優先度内での優先順位が変わらないこと
 *		(B-3) ミューテックスXに，タスクAより低い現在優先度のタスクBが
 *			  待ち状態となっている場合に，タスクBの強制的な待ち解除で
 *			  タスクAの現在優先度が変わらないこと．また，同じ優先度内
 *			  での優先順位が変わらないこと
 *	(C) タスクA（優先度継承ミューテックスYに対するロック待ち状態）がロッ
 *		クしている優先度継承ミューテックスXに対してロック待ち状態となっ
 *		ているタスクBを強制的に待ち解除（rel_wai）したことに伴う，タス
 *		クAの優先度制御
 *		(C-1) ミューテックスXに，タスクAと同じ現在優先度のタスクBが待
 *			  ち状態となっており，タスクBの強制的な待ち解除でタスクAの
 *			  現在優先度が下がるべき場合に，現在優先度が適切に下げられ
 *			  ること．また，待ち行列中で同じ優先度内での順序が最後にな
 *			  ること
 *		(C-2) ミューテックスXに，タスクAと同じ現在優先度のタスクBが待
 *			  ち状態となっており，タスクBの強制的な待ち解除でタスクAの
 *			  現在優先度が下がるべき場合に，現在優先度が適切に下げられ
 *			  ること．また，待ち行列中で同じ優先度内での順序が最後にな
 *			  ること．さらに，ミューテックスYをロックしているタスクCに
 *			  対して，推移的な優先度継承が起こること
 *		(C-3) ミューテックスXに，タスクAと同じ現在優先度のタスクBが待
 *			  ち状態となっており，タスクBの強制的な待ち解除でタスクAの
 *			  現在優先度が変わるべきでない場合に，現在優先度が変わらな
 *			  いこと．また，待ち行列中で順序が変わらないこと
 *		(C-4) ミューテックスXに，タスクAより低い現在優先度のタスクBが
 *			  待ち状態となっている場合に，タスクBの強制的な待ち解除で
 ...
 *		にrel_wai(TASK6)を発行させ，TASK1が中優先度になることを確認す
 *		る．また，他の中優先度のタスク（TASK2）を実行できる状態にして
 *		おき，それよりも優先順位が高くなることを確認する．
 *	(A-2)
 *		優先度継承ミューテックスX（MTX1）を，中優先度タスクA（TASK1）
 *		がロックし，中優先度タスクB（TASK2）が待っている状態で，TASK1
 *		にrel_wai(TASK2)を発行させ，TASK1が中優先度のままであることを
 *		確認する．また，実行可能状態の中優先度のタスク2つ（TASK3，
 *		TASK4）を用意にしておき，TASK1の優先順位が変わらないことを確認
 *		する．ディスパッチ禁止状態で実施する．
 *	(A-3)
 *		優先度継承ミューテックスX（MTX1）を，中優先度タスクA（TASK1）
 *		がロックし，低優先度タスクB（TASK8）が待っている状態で，TASK1
 *		にrel_wai(TASK8)を発行させ，TASK1が中優先度のままであることを
 *		確認する．また，実行可能状態の中優先度のタスク2つ（TASK3，
 *		TASK4）を用意にしておき，TASK1の優先順位が変わらないことを確認
 *		する．ディスパッチ禁止状態で実施する．
 *	(B-1)
 ...
 *		にrel_wai(TASK6)を発行させ，TASK1が中優先度になることを確認す
 *		る．また，他の中優先度のタスク（TASK3）を実行できる状態にして
 *		おき，それよりも優先順位が高くなることを確認する．
 *	(B-2)
 *		優先度継承ミューテックスX（MTX1）を，中優先度タスクA（TASK1）
 ...
 *		状態にする．TASK7にrel_wai(MTX5)を発行させ，TASK1が中優先度に
 ...
 *		rel_wai(TASK6)を発行させ，TASK1が中優先度になることを確認する．
 ...
 *		rel_wai(TASK6)を発行させ，TASK1の現在優先度が変わらないことを
 *		確認する．また，MTX2に対する待ち状態の中優先度タスク2つ（TASK3，
 ...
 *		rel_wai(TASK2)									... (A-2)
 *	//		中：TASK3→TASK1→TASK4→TASK2，MTX1：TASK1
 *		ref_tsk(TASK1, &rtsk)
 *		assert(rtsk.tskpri == MID_PRIORITY)
 *		ena_dsp()
 *	== TASK3-1（優先度：中）==
 *	10:	ext_tsk()
 *	//		中：TASK1→TASK4→TASK2，MTX1：TASK1
 *	== TASK1（続き）==
 *	11:	rot_rdq(MID_PRIORITY)
 *	//		中：TASK4→TASK2→TASK1，MTX1：TASK1
 *	== TASK4-1（優先度：中）==
 *	12:	ext_tsk()
 *	//		中：TASK2→TASK1，MTX1：TASK1
 *	== TASK2-1（続き）==
 ...
 *		rel_wai(TASK8)									... (A-3)
 *	//		中：TASK3→TASK1→TASK4，低：TASK8，MTX1：TASK1
 *		ref_tsk(TASK1, &rtsk)
 *		assert(rtsk.tskpri == MID_PRIORITY)
 *		ena_dsp()
 *	== TASK3-2（優先度：中）==
 *	18:	ext_tsk()
 *	//		中：TASK1→TASK4，低：TASK8，MTX1：TASK1
 *	== TASK1（続き）==
 *	19:	chg_pri(TASK1, LOW_PRIORITY)
 *	//		中：TASK4，低：TASK8→TASK1，MTX1：TASK1
 *	== TASK4-2（優先度：中）==
 *	20:	ext_tsk()
 *	//		低：TASK8→TASK1，MTX1：TASK1
 *	== TASK8-1（続き）==
 *	21:	ext_tsk()
 *	//		低：TASK1，MTX1：TASK1
 *	== TASK1（続き）==
 ...
 *	== TASK7-1（優先度：高）==
 *	26:	rel_wai(TASK6)									... (B-1)
 *	//		高：TASK7→TASK6，中：TASK1→TASK3，MTX1：TASK1
 *		ref_tsk(TASK1, &rtsk)
 *		assert(rtsk.tskpri == MID_PRIORITY)
 *		ext_tsk()
 *	//		高：TASK6，中：TASK1→TASK3，MTX1：TASK1
 *	== TASK6-2（続き）==
 *	27:	ext_tsk()
 ...
 *	== TASK7-2（優先度：高）==
 *	34:	rel_wai(TASK2)									... (B-2)
 *	//		高：TASK7，中：TASK3→TASK1→TASK4→TASK2，MTX1：TASK1
 *		ref_tsk(TASK1, &rtsk)
 *		assert(rtsk.tskpri == MID_PRIORITY)
 *		ext_tsk()
 *	//		中：TASK3→TASK1→TASK4→TASK2，MTX1：TASK1
 ...
 *	== TASK7-3（優先度：高）==
 *	45:	rel_wai(TASK8)									... (B-3)
 *	//		高：TASK7，中：TASK3→TASK1→TASK4，低：TASK8，MTX1：TASK1
 *		ref_tsk(TASK1, &rtsk)
 *		assert(rtsk.tskpri == MID_PRIORITY)
 *		ext_tsk()
 *	//		中：TASK3→TASK1→TASK4，低：TASK8，MTX1：TASK1
 ...
 *	== TASK2-3（優先度：中）==
 *	52:	loc_mtx(MTX2)
 *	//		中：TASK2→TASK1，MTX1：TASK1，MTX2：TASK2
 *		act_tsk(TASK3)
 *	//		中：TASK2→TASK1→TASK3，MTX1：TASK1，MTX2：TASK2
 *		rot_rdq(MID_PRIORITY)
 ...
 *		loc_mtx(MTX1) -> E_RLWAI
 *	//		高：TASK7→TASK2，MTX1：TASK1→TASK6，MTX2：TASK2→TASK1→TASK3
 *	//		優先度継承により，TASK1が高優先度になる。
 *	== TASK7-4（優先度：高）==
 *	57:	rel_wai(TASK6)									... (C-1)
 *	//		高：TASK7→TASK2→TASK6，MTX1：TASK1，MTX2：TASK2→TASK3→TASK1
 *	//		優先度継承の解除により，TASK1が中優先度になる。
 *		ref_tsk(TASK1, &rtsk)
 *		assert(rtsk.tskpri == MID_PRIORITY)
 *		ext_tsk()
 *	//		高：TASK2→TASK6，MTX1：TASK1，MTX2：TASK2→TASK3→TASK1
 *	== TASK2-3（続き）==
 *	58:	unl_mtx(MTX2)
 *	//		中：TASK2→TASK6→TASK3，MTX1：TASK1，MTX2：TASK3→TASK1
 *		ext_tsk()
 *	//		中：TASK6→TASK3，MTX1：TASK1，MTX2：TASK3→TASK1
 *	== TASK6-3（続き）==
 *	59:	ext_tsk()
 *	//		中：TASK3，MTX1：TASK1，MTX2：TASK3→TASK1
 *	== TASK3-6（続き）==
 *	60:	unl_mtx(MTX2)
 *	//		中：TASK3→TASK1，MTX1：TASK1，MTX2：TASK1
 *		ext_tsk()
 *	//		中：TASK1，MTX1：TASK1，MTX2：TASK1
 *	== TASK1（続き）==
 *	61:	unl_mtx(MTX2)
 *	//		中：TASK1，MTX1：TASK1
 *
 ...
 *	//		高：TASK7→TASK2，MTX1：TASK1→TASK6，MTX2：TASK2→TASK1→TASK3
 *	== TASK7-5（優先度：高）==
 *	69:	rel_wai(TASK6)									... (C-2)
 *	//		高：TASK7→TASK6，中：TASK2，MTX1：TASK1，MTX2：TASK2→TASK3→TASK1
 ...
 *		assert(rtsk.tskpri == MID_PRIORITY)
 *		ext_tsk()
 *	//		高：TASK6，中：TASK2，MTX1：TASK1，MTX2：TASK2→TASK3→TASK1
 *	== TASK6-4（続き）==
 *	70:	ext_tsk()
 *	//		中：TASK2，MTX1：TASK1，MTX2：TASK2→TASK3→TASK1
 *	== TASK2-4（続き）==
 *	71:	unl_mtx(MTX2)
 *	//		中：TASK2→TASK3，MTX1：TASK1，MTX2：TASK3→TASK1
 *		ext_tsk()
 *	//		中：TASK3，MTX1：TASK1，MTX2：TASK3→TASK1
 *	== TASK3-7（続き）==
 *	72:	unl_mtx(MTX2)
 ...
 *	== TASK2-5（優先度：中）==
 *	75:	loc_mtx(MTX2)
 *	//		中：TASK2→TASK3→TASK1，MTX1：TASK1，MTX2：TASK2
 *		rot_rdq(MID_PRIORITY)
 *	//		中：TASK3→TASK1→TASK2，MTX1：TASK1，MTX2：TASK2
 *	== TASK3-8（優先度：中）==
 *	76:	loc_mtx(MTX2)
 ...
 *	== TASK2-5（続き）==
 *	84:	unl_mtx(MTX2)
 *	//		中：TASK2→TASK5→TASK3，MTX1：TASK1，MTX2：TASK3→TASK1→TASK4
 *		ext_tsk()
 *	//		中：TASK5→TASK3，MTX1：TASK1，MTX2：TASK3→TASK1→TASK4
 *	== TASK5-1（続き）==
 *	85:	ext_tsk()
 *	//		中：TASK3，MTX1：TASK1，MTX2：TASK3→TASK1→TASK4
 *	== TASK3-8（続き）==
 *	86:	unl_mtx(MTX2)
 *	//		中：TASK3→TASK1，MTX1：TASK1，MTX2：TASK1→TASK4
 *		ext_tsk()
 *	//		中：TASK1，MTX1：TASK1，MTX2：TASK1→TASK4
 *	== TASK1（続き）==
 *	87:	unl_mtx(MTX2)
 *	//		中：TASK1→TASK4，MTX1：TASK1，MTX2：TASK4
 *	88:	rot_rdq(MID_PRIORITY)
 *	//		中：TASK4→TASK1，MTX1：TASK1，MTX2：TASK4
 *	== TASK4-5（続き）==
 *	89:	unl_mtx(MTX2)
 *	//		中：TASK4→TASK1，MTX1：TASK1
 *		ext_tsk()
 *	//		中：TASK1，MTX1：TASK1
 *
 *	== TASK1（続き）==
 ...
 *	== TASK7-7（優先度：高）==
 *	100:rel_wai(TASK8)								... (C-4)
 *	//		高：TASK7，中：TASK2，低：TASK8，MTX1：TASK1，MTX2：TASK2→TASK3→TASK1→TASK4
 *		ref_tsk(TASK1, &rtsk)
 *		assert(rtsk.tskpri == MID_PRIORITY)
 *		ext_tsk()
 *	//		中：TASK2，低：TASK8，MTX1：TASK1，MTX2：TASK2→TASK3→TASK1→TASK4
 *	== TASK2-6（続き）==
 *	101:unl_mtx(MTX2)
 *	//		中：TASK2→TASK3，低：TASK8，MTX1：TASK1，MTX2：TASK3→TASK1→TASK4
 *		ext_tsk()
 *	//		中：TASK3，低：TASK8，MTX1：TASK1，MTX2：TASK3→TASK1→TASK4
 *	== TASK3-9（続き）==
 *	102:unl_mtx(MTX2)
 *	//		中：TASK3→TASK1，低：TASK8，MTX1：TASK1，MTX2：TASK1→TASK4
 *		ext_tsk()
 *	//		中：TASK1，低：TASK8，MTX1：TASK1，MTX2：TASK1→TASK4
 *	== TASK1（続き）==
 *	103:unl_mtx(MTX2)
 *	//		中：TASK1→TASK4，低：TASK8，MTX1：TASK1，MTX2：TASK4
 *		chg_pri(TASK1, LOW_PRIORITY)
 *	//		中：TASK4，低：TASK8→TASK1，MTX1：TASK1，MTX2：TASK4
 *	== TASK4-6（続き）==
 *	104:unl_mtx(MTX2)
 *	//		中：TASK4，低：TASK8→TASK1，MTX1：TASK1
 *		ext_tsk()
