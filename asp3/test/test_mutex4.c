 ...
 *  の責任を負わない．
 * 
 *  $Id: test_mutex4.c 1610 2022-09-16 16:42:29Z ertl-hiro $
 */
 ...
 * 【テスト項目】
 *
 *	(A) loc_mtx/ploc_mtxにより，自タスクが優先度上限ミューテックスをロッ
 *		クしたことに伴う，自タスクの優先度制御
 *		(A-1) ロックしたミューテックスの優先度上限が自タスクの現在優先
 *			  度よりも高い場合に，自タスクの現在優先度がその優先度上限
 *			  まで上がること．また，同じ優先度内での優先順位が最高にな
 *			  ること
 *		(A-2) ロックしたミューテックスの優先度上限が自タスクの現在優先
 *			  度と同じ場合に，自タスクの現在優先度が変わらないこと．ま
 *			  た，同じ優先度内での優先順位が変わらないこと
 *		(A-3) ロックしたミューテックスの優先度上限が自タスクの現在優先
 *			  度よりも低い場合に，自タスクの現在優先度が変わらないこと．
 *			  また，同じ優先度内での優先順位が変わらないこと
 *	(B) unl_mtxにより，自タスクが優先度上限ミューテックスをロック解除
 * 		したことに伴う，自タスクの優先度制御
 *		(B-1) ロック解除したミューテックスの上限優先度が自タスクの現在
 *			  優先度と同じで，ミューテックスのロック解除で現在優先度が
 *			  下がるべき場合に，自タスクの現在優先度が適切に下げられる
 *			  こと．また，同じ優先度内での優先順位が最高になること
 *		(B-2) ロック解除したミューテックスの上限優先度が自タスクの現在
 *			  優先度と同じで，ミューテックスのロック解除で現在優先度が
 *			  変わるべきでない場合に，自タスクの現在優先度が変わらない
 *			  こと．また，同じ優先度内での優先順位が変わらないこと
 *		(B-3) ロック解除したミューテックスの上限優先度が自タスクの現在
 *			  優先度よりも低い場合に，自タスクの現在優先度が変わらない
 *			  こと．また，同じ現在優先度内での優先順位が変わらないこと
 *	(C) unl_mtxにより，優先度上限ミューテックスをロック解除したことに
 * 		伴う，ミューテックスをロックしたタスクの優先度制御
 *		(C-1) ミューテックスの優先度上限が新たにミューテックスをロック
 *			  したタスクの現在優先度よりも高い場合に，当該タスクの現在
 *			  優先度がその優先度上限まで上がること．また，同じ優先度内
 *			  での優先順位が最低になること
 *		(C-2) ミューテックスの優先度上限が新たにミューテックスをロック
 *			  したタスクの現在優先度と同じ場合に，当該タスクの現在優先
 *			  度が変わらないこと．また，同じ優先度内での優先順位が最低
 *			  になること
 *		(C-3) ミューテックスの優先度上限が新たにミューテックスをロック
 *			  したタスクの現在優先度よりも低い場合に，当該タスクの現在
 *			  優先度が変わらないこと．また，同じ優先度内での優先順位が
 *			  最低になること
