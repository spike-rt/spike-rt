 ...
 *  の責任を負わない．
 * 
 *  $Id: mutex.h 1636 2022-09-23 10:08:01Z ertl-hiro $
 */
 ...
 *
 *  ミューテックス待ち状態のタスクの現在優先度が変更された際に呼び出す
 *  関数であり，ミューテックス待ちキューの中での位置を修正し，必要な優
 *  先度制御処理を行う．推移的な優先度継承が必要な場合には，優先度を継
 *  承すべきタスクのTCBへのポインタを返し，*p_newpriに継承すべき優先度
 *  を代入する．推移的な優先度継承が必要そうでない場合には，NULLを返す．
 */
extern TCB *mutex_change_priority(TCB *p_tcb, uint_t oldpri, uint_t *p_newpri);
 ...
 *
 *  p_tcbで指定されるタスクをミューテックス待ち状態から解除する際に呼び
 *  出す関数であり，必要な場合には，そのミューテックスをロックしている
 *  タスクの優先度を変更する．
 */
extern void mutex_dequeue_wobj(TCB *p_tcb);
